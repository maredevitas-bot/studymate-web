...
export default function MaterialCard({ material }: Props) {
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm space-y-2">
      ...
      {material.summary && (
        <p className="text-sm text-gray-600 leading-relaxed line-clamp-3">{material.summary}</p>
      )}
      {material.highlights?.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {material.highlights.map((h, i) => (
            <span key={i} className="bg-blue-50 text-blue-600 text-xs px-2 py-0.5 rounded-full">{h}</span>
          ))}
        </div>
      )}
    </div>
