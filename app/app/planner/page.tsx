  ...
  return (
    <div className="pt-8 space-y-6">
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm space-y-3">
        <p className="font-medium text-sm">AI 플래너 생성</p>
        ...
        <div key={date} className="space-y-2">
          ...
          {grouped[date].map((e: any) => (
            <div key={e.id} onClick={() => handleToggle(e.id, e.completed)}
              className={`bg-white rounded-xl p-4 shadow-sm flex items-start gap-3 cursor-pointer transition-opacity
                ${e.completed ? 'opacity-50' : ''}`}>
              <span className="text-lg">{e.completed ? '✅' : '⬜'}</span>
              <div>
                <p className="font-medium text-sm">{e.subject_name}</p>
                <p className="text-xs text-gray-500 mt-0.5">{e.topic}</p>
              </div>
            </div>
          ))}
        </div>
