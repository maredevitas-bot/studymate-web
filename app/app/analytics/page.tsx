    ...
    <div className="pt-8 space-y-6">
      ...
      {subjectStats.length === 0 ? (
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
          <h2 className="font-semibold">과목별 평균 점수</h2>
          {subjectStats.map(s => (
            <div key={s.id} className="space-y-1">
              <div className="flex justify-between text-sm">
                <span className={s.avg! < 70 ? 'text-red-500 font-medium' : 'text-gray-700'}>{s.name}</span>
                <span className={s.avg! < 70 ? 'text-red-500 font-bold' : 'text-green-500 font-bold'}>
                  {Math.round(s.avg!)}점
                </span>
              </div>
      ...
      {completionRate !== null && (
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          <h2 className="font-semibold mb-4">플래너 완료율</h2>
