  ...
  return (
    <div className="pt-8 space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/subjects" className="text-gray-400">‹</Link>
        <div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-white">자료 ({materials.length})</h2>
          <Link href={`/subjects/${subject.id}/material/new`}
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-white">퀴즈</h2>
          {materials.length > 0 && (
            <Link href={`/subjects/${subject.id}/quiz/setup`}
              className="text-sm text-blue-500">+ 시작</Link>
          )}
        </div>
        {sessions.slice(0, 5).map((s: any) => (
          <div key={s.id} className="bg-gray-800 rounded-2xl p-4 shadow-sm flex justify-between items-center">
